...
def show_summary_submenu() -> str:
    """Показывает подменю для промпта 'Резюме текста'"""
    print("\n" + "="*80)
    print("📋 ПОДМЕНЮ: Резюме текста")
    print("="*80)
    print("\n1. Стандартные запросы (резюмирование)")
    print("2. Генерация текстового поста (LangChain)")
...
def generate_text_post():
    """Запускает генератор текстовых постов"""
    print("\n" + "="*80)
    print("📝 ГЕНЕРАТОР ТЕКСТОВЫХ ПОСТОВ")
    print("="*80)
    print("\nС помощью LangChain будет создан готовый пост на основе вашей темы.")
    print("Цепочка обработки: Анализ → Подбор стиля → Структура → Генерация")
    print()
    
    # Получаем тему поста
    topic = get_multiline_input("💬 Введите тему поста:")
    
    if not topic:
        print("❌ Тема не может быть пустой!")
        return
    
    # Спрашиваем про исходный материал
    has_source = yes_no_question("Есть ли исходный текст/статья для резюмирования?", "n")
    
    source_text = ""
    if has_source:
        source_text = get_multiline_input("📄 Вставьте исходный текст:")
    
    # Проверяем наличие script_post.py
    script_path = Path("script_post.py")
    if not script_path.exists():
        print("❌ Файл script_post.py не найден!")
        return
    
    print("\n⏳ Запускаем генератор постов...")
    print("-" * 80)
    
    try:
        # Запускаем script_post.py с темой и текстом
        cmd = [sys.executable, "script_post.py", topic]
        if source_text:
            cmd.append(source_text)
        ...
        if result.returncode == 0:
            ...
            print(f"\n❌ Ошибка при генерации поста (код: {result.returncode})")
    ...
    while True:
        ...
        try:
            ...
            if selected_prompt:
                ...
                # Проверяем, является ли это промптом "Резюме текста"
                if selected_prompt.get('prompt_id') == 'summary':
                    submenu_choice = show_summary_submenu()
                ...
                elif selected_prompt.get('prompt_id') == 'code_structure':
